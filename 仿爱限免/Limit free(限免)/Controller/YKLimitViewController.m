...
@interface YKLimitViewController ()<UISearchBarDelegate>
...
- (instancetype)init {
    if (self = [super init]) {
        _currentPage = 0;    // 初始化为0
        _isFirstShow = YES;  // 初始化为yes
    }
    return self;
}
...
- (void)loadMoreDatas {
    ...
    _currentPage ++; // 每次加载数据，页数加1
    
    NSString *urlStr = [NSString stringWithFormat:LIMIT_URL,_currentPage];
    
    [manager GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        //YKLog(@"res = %@", responseObject);
...
- (NSArray *)statusFramesWithStatuses:(NSArray *)statuses {
    ...
    for (YKStatuses *status in statuses) {
        YKStatusFrame *frame = [[YKStatusFrame alloc] init];
        frame.status = status;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 获得自定义cell
    YKStatusViewCell *cell = [YKStatusViewCell cellWithTableView:tableView];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    YKStatusFrame *frame = self.statusFrames[indexPath.row];
